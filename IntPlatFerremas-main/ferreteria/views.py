...
from .api import BancoCentralAPI
...
#Api banco central
def banco_central_view(request):
    user = "marcela.segovia0405@gmail.com"
    password = "Luna.0405"
    series_id = "F073.UFF.PRE.Z.D"

    # Fechas: Desde el 1 de enero de 2024 hasta hoy
    start_date = "2024-01-01"
    end_date = datetime.now().strftime("%Y-%m-%d")

    api = BancoCentralAPI(user, password)
    data = api.obtener_valores(series_id, start_date, end_date)

    if data:
        return JsonResponse(data, safe=False)
    else:
        return JsonResponse({"error": "Error al obtener datos del Banco Central"}, status=500)